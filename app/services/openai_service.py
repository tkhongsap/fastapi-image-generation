...
from utils.openai_client import initialize_openai_client, get_client, get_active_model, is_fallback_mode, reinitialize_client_if_needed
...
class OpenAIService:
    ...
    def __init__(self):
        """Initialize the OpenAI client using the utility."""
        self.client, self.active_model, self.fallback = initialize_openai_client()
        self.initialized = self.client is not None and not self.fallback
    
    def _ensure_client(self):
        if not self.initialized or self.client is None or is_fallback_mode():
            self.client, self.active_model, self.fallback = initialize_openai_client()
            self.initialized = self.client is not None and not self.fallback
    ...
    async def generate_image(
        ...
        response = await self._with_retries(
            lambda: self.client.images.generate(**params)
        )
        result = {
            "model": model.value,
            "images": [],
    ...
    async def edit_image(
        ...
        model: ImageModel = ImageModel.DALL_E_2,
        n: int = 1,
        size: ImageSize = ImageSize.LARGE,
        response_format: ImageFormat = ImageFormat.PNG,
    ) -> Dict[str, Any]:
        ...
        if mask:
            params["mask"] = mask
        response = await self._with_retries(
            lambda: self.client.images.edits.create(**params)
        )
        result = {
            "model": model.value,
            "images": [],
    ...
    async def create_variation(
        ...
        response = await self._with_retries(
            lambda: self.client.images.variations.create(**params)
        )
        result = {
            "model": model.value,
            "images": [],
